...
bool CRulesModel::loadRule( const COutlookObj< Outlook::Rule > &rule, QStandardItem *ruleItem )
{
    if ( !rule )
        return false;
...
COutlookObj< Outlook::Rule > CRulesModel::getRule( const QModelIndex &index ) const
{
    auto item = itemFromIndex( index );
    return getRule( item );
}

COutlookObj< Outlook::Rule > CRulesModel::getRule( const QStandardItem *item ) const
{
...
void CRulesModel::slotRuleAdded( const COutlookObj< Outlook::Rule > &rule )
{
    if ( !rule )
        return;
    loadRule( rule );
...
void CRulesModel::slotRuleChanged( const COutlookObj< Outlook::Rule > &rule )
{
    if ( !rule )
        return;
    updateRule( rule );
...
void CRulesModel::slotRuleDeleted( const COutlookObj< Outlook::Rule > &rule )
{
    if ( !rule )
        return;
...
bool CRulesModel::updateRule( const COutlookObj< Outlook::Rule > &rule )
{
