...
{
    str << "    " << gOptions.enumToken() << " " << metaEnum.name() << " {" << Qt::endl;
    for ( int k = 0, last = metaEnum.keyCount() - 1; k <= last; ++k )
    {
        ...
        if ( k < last )
            str << ',';
        str << Qt::endl;
        updateEnumMap( key, metaEnum.name() );
    }
    str << "    };" << Qt::endl;
}

void formatCppEnums( QTextStream &str, const QMetaObject *mo, const char *qEnumDecl = nullptr /* Q_ENUM, Q_ENUM_NS */ )
{
...
static void formatCppMethods( QTextStream &str, const QMetaObject *mo, QMetaMethod::MethodType filter )
{
...
{
    str << "    Q_PROPERTY(" << p.typeName() << ' ' << p.name() << " READ " << p.name();
    if ( p.isWritable() )
...
static QString mocHeader( const QMetaObject *mo, const QStringList &name, const QString &baseClass )
{
    QString result;
...
static QString runProcess( const QString &binary, const QStringList &args, QString *errorString )
{
    QProcess process;
...
static void removeLines( const QString &start, const QString &end, QString *s, bool keepEnd = false )
{
    ...
    endPos = keepEnd ? lineStart( endPos, s ) : nextLineFeed( endPos + end.size(), s );
...
static QString cleanCode( QString code, const QString &className, const QString &headerFileName )
{
    ...
    if ( lastSep >= 0 )
        ...
        parentName.clear();
    code.replace( enumPrefix + className + QLatin1String( "::" ), enumPrefix + parentName + QLatin1String( "::" ) );
    return code;
}

QString mocCode( const QMetaObject *mo, const QString &qualifiedClassName, QString *errorString )
{
    ...
    const QString tempPattern = QDir::tempPath() + QLatin1Char( '/' ) + name.constLast().toLower() + QLatin1String( "_XXXXXX.h" );
    ...
    QString result = runProcess( gOptions.mocExecPath, { header.fileName() }, errorString );
    if ( result.isEmpty() )
    {
