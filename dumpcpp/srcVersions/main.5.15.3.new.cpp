...
std::optional< QString > generateFromString( const QMetaEnum &metaEnum, const QString &nameSpace )
{
    ...
    out << getFromStringDecl( enumName, nameSpace ) << Qt::endl   //
        << "{" << Qt::endl
    ...
    out << Qt::endl
        << "    auto pos = sEnumMap.find( " << getValueNameForEnum( enumName ) << "Str.toLower() );" << Qt::endl   //
        ...
        if ( thisEnumCount )
        {
            for ( auto ii = metaObject->enumeratorOffset(); ii < allEnumCount; ++ii )
            {
                ...
                if ( mo.isValid() )
                {
                    ...
                    {
                        out << "        case " << mo.key( j ) << ": return \"" << getEnumString( mo.key( j ) ) << "\";" << Qt::endl;
                    }
                    out << QString( R"(        default: return "<UNKNOWN-%1>";)" ).arg( enumName ) << Qt::endl   //
                        << "    };" << Qt::endl
                        << "};" << Qt::endl;
...
void writeEnums( QTextStream &out, const QMetaObject *mo, const QString &nameSpace )
{
    ...
    for ( int ienum = mo->enumeratorOffset(); ienum < mo->enumeratorCount(); ++ienum )
    {
        ...
        out << "    " << sOptions.enumToken() << " " << metaEnum.name() << " {" << Qt::endl;
        ...
        if ( sOptions.generateToFromEnum )
        {
            if ( !sToStringDeclMap.contains( metaEnum.name() ) )
            {
                ...
                out << "    " << getToStringDecl( metaEnum.name() ) << ";" << Qt::endl;
                out << "    " << getFromStringDecl( metaEnum.name() ) << ";" << Qt::endl;
            }
...
static void formatConstructorBody( QTextStream &out, const QByteArray &className, const QString &controlID, ObjectCategories category )
{
...
#define addStringIdx( string ) out << stridx( string ) << ", ";
...
int nameToBuiltinType( const QByteArray &name )
{
    ...
    const int tp = QMetaType::fromName( name.constData() ).id();
...
void generateClassImpl( QTextStream &out, const QString &controlID, const QMetaObject *mo, const QByteArray &className, const QByteArray &nameSpace, ObjectCategories category )
{
    ...
    if ( thisPropertyCount )
    {
        ...
        for ( int i = mo->propertyOffset(); i < allPropertyCount; ++i )
        {
            ...
            int t = property.userType();
            const auto vartype = t >= QMetaType::User ? QMetaType::User : QMetaType::Type( t );
            if ( vartype != QMetaType::UnknownType && vartype != QMetaType::User )
            ...
            out << "0x" << QString::number( flags, 16 ).rightJustified( 8, QLatin1Char( '0' ) ) << ", \t\t // " << property.typeName() << ' ' << property.name() << Qt::endl;
        }
        out << Qt::endl;
    }
    ...
    if ( thisEnumCount )
    {
        ...
        for ( int i = mo->enumeratorOffset(); i < allEnumCount; ++i )
        {
            ...
            for ( int j = 0; j < enumerator.keyCount(); ++j )
            {
                ...
                out << "uint(";
                auto key = QString( nameSpace.isEmpty() ? className : nameSpace );
                key += "::" + QString( enumerator.key( j ) );
                auto pos = sEnumMap.find( key );
                if ( pos != sEnumMap.end() )
                    key = ( *pos ).second;
...
bool generateTypeLibrary( QString typeLibFile )
{
    ...
    for ( UINT index = 0; index < typeCount; ++index )
    {
        ...
        if ( metaObject )
        {
            ...
            else
            {
                ...
                if ( implFile.isOpen() )
                    generateClassImpl( classImplOut, guid.toString(), metaObject, className, libNameBa, object_category );
            }
    ...
    if ( !strings.isEmpty() && implFile.isOpen() )
    {
        ...
        implOut << "#define QT_MOC_LITERAL(idx, ofs, len, table) \\" << Qt::endl << "    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \\" << Qt::endl << "    offsetof(qt_meta_stringdata_all_t, stringdata##table) + ofs \\" << Qt::endl << "        - idx * sizeof(QByteArrayData) \\" << Qt::endl << "    )" << Qt::endl;
